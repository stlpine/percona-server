...
// mvcc_filter.c — CEMU Computational Storage Function (CSF)
//
// C port of mvcc_filter.cc. The original .cc file uses no real C++ features
// (no classes, templates, STL, exceptions, or overloading) — just
// static_cast, nullptr, and default member initializers — so this port is a
// mechanical syntax change, not a redesign. FEMU dlopen()/dlsym()s this by
// plain symbol name; a C build never mangles names, so no extern "C" is
// needed here (that wrapper only exists in the .cc file to suppress C++
// name-mangling in the first place).
//
...
//       -DHAVE_CEMU_SDK \
//       -I${CEMU_SRC}/include \
//       -o mvcc_filter.so mvcc_filter.c
...
#define CEMU_CSF_ENTRY(name) long long name
...
static bool parse_metaindex_for_index(const char *block_data, size_t block_size,
  ...
  while (p < entry_end) {
    ...
    if (full_key_len == kIndexBlockKeyLen &&
      ...
      return decode_block_handle(val_ptr, val_ptr + val_len, index_bh) != NULL;
    }
  }
  return false;
}
...
struct FilterState {
  ...
  size_t key_buf_len;      // resets at each block boundary
...
static void process_data_block(const char *block_data, size_t block_size,
                               uint64_t snapshot_seq, struct FilterState *st) {
  ...
  while (p < entry_end && !st->overflow) {
    ...
    if ((size_t)(entry_end - p) < non_shared + val_len) break;
    ...
    const uint8_t type = (uint8_t)(packed & 0xff);
    ...
    if ((size_t)(st->out_end - st->out_ptr) < needed) {
      st->overflow = true;
      break;
    }

    write_uint32_le(st->out_ptr, (uint32_t)full_key_len);
    st->out_ptr += 4;
    memcpy(st->out_ptr, st->key_buf, full_key_len);
    st->out_ptr += full_key_len;
    write_uint32_le(st->out_ptr, (uint32_t)val_len);
    st->out_ptr += 4;
...
CEMU_CSF_ENTRY(mvcc_filter)(struct cemu_args *args) {
  const char *file_data = (const char *)(args->mr_addr[0]);
  ...
  size_t file_len = (args->cparam2 > 0) ? (size_t)(args->cparam2)
                                        : (size_t)(args->mr_len[0]);
  char *out_buf = (char *)(args->mr_addr[1]);
  const size_t out_capacity = (size_t)(args->mr_len[1]);
  const uint64_t snapshot_seq = (uint64_t)(args->cparam1);
  ...
  while (p < idx_entry_end && !st.overflow) {
    ...
    if ((size_t)(idx_entry_end - p) < non_shared) break;
  ...
  const uint64_t result_bytes = (uint64_t)(st.out_ptr - (out_buf + kHeaderSize));
