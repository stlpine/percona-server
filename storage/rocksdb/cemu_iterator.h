...
// and written to the FDM (on-device DRAM) output buffer.
//
// Stream format:  [uint32 key_len][key bytes][uint32 val_len][value bytes] ...
//
// The buffer is already MVCC-filtered: every entry is visible to the snapshot
// and is the newest visible version of its user_key. DBIter can therefore skip
// its own internal_key_skipped_count bookkeeping for this iterator's output.
//
// Forward-only: Prev()/SeekToLast()/SeekForPrev() are not implemented because
// the CSF scans the SST in forward order and writes results sequentially.
// Callers that require reverse iteration must use the unfiltered inner reader.
class CemuResultIterator : public rocksdb::InternalIterator {
 public:
  // Takes ownership of buf (heap-allocated, must be freed with delete[]).
  // keys_seen / keys_filtered come from the execute result returned by CEMU.
  CemuResultIterator(char *buf, size_t len, uint64_t keys_seen,
  ...
  ~CemuResultIterator() override {
    myrocks::rocksdb_cemu_keys_seen.fetch_add(keys_seen_,
                                              std::memory_order_relaxed);
    myrocks::rocksdb_cemu_keys_filtered.fetch_add(keys_filtered_,
                                                  std::memory_order_relaxed);
    delete[] buf_;
  }
