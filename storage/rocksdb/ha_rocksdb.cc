...
bool rocksdb_cemu_enabled = false;
std::atomic<uint64_t> rocksdb_cemu_keys_seen{0};
std::atomic<uint64_t> rocksdb_cemu_keys_filtered{0};
std::atomic<uint64_t> rocksdb_cemu_freeze_ns{0};
...
static MYSQL_SYSVAR_BOOL(
    cemu_enabled, rocksdb_cemu_enabled, PLUGIN_VAR_RQCMDARG,
    "Enable MVCC filter pushdown via CEMU emulated CSD for user-facing scans "
    "on column families whose name begins with 'csd_'. When ON, the "
    "mvcc_filter "
    "CSF runs on the emulated compute unit and only filtered key-value pairs "
    "are "
    "returned to the host. Requires -DHAVE_CEMU build and a running CEMU VM.",
    nullptr, nullptr, false);
...
static SHOW_VAR rocksdb_status_vars[] = {
    ...
    DEF_STATUS_VAR_PTR("cemu_keys_seen", &rocksdb_cemu_keys_seen,
                       SHOW_LONGLONG),
    DEF_STATUS_VAR_PTR("cemu_keys_filtered", &rocksdb_cemu_keys_filtered,
                       SHOW_LONGLONG),
    DEF_STATUS_VAR_PTR("cemu_freeze_ns", &rocksdb_cemu_freeze_ns,
                       SHOW_LONGLONG),
